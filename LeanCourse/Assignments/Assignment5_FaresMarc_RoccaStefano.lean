...
lemma sum_cube_eq_sq_sum (n : ℕ) :
  ...
  | succ n ih => {
    ...
    have sumn : (∑ i ∈ Finset.range (n + 1), (i : ℚ)) = (n:ℚ) * (n + 1:ℚ) / 2  := by apply /-SumofFirstn-/ sorry
...
lemma disjoint_unions {ι α : Type*} [LinearOrder ι] [wf : WellFoundedLT ι] (A C : ι → Set α)
  ...
  · unfold Pairwise
    ...
    have : (C i) ∩ (C k) = ∅ := by {
      ...
      simp
      intro hhC
      have hC₁ : ∀ (i : ι), C i = A i \ ⋃ j, ⋃ (_ : j < i), A j := by exact hC
      specialize hC i
      specialize hC₁ k
      rw[hC] at hhC
      rw[hC₁]
