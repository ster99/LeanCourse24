...
example (a b c d : ℝ) : a + b + c + d = d + (b + a) + c := by {
  /-I used the ring axioms below, otherwise it would have been a proof by ring -/
  calc
    a + b + c + d = c + d + (a + b) := by rw[add_assoc, add_comm]
                _ = d + c + (b + a) := by rw[add_comm c, add_comm b]
                _ = d + (b + a) + c := by rw[ add_assoc d, add_comm c, add_assoc d]
}
...
example : (a + b) * (a - b) = a^2 - b^2 := by {
  ...
  rw[add_comm, add_sub, add_comm, mul_comm]
  rw[← add_sub]
  rw[sub_self]
}
